...
case object RecBus extends Field[MasterPortParams]
...
trait HasMasterAXI4RecoveryPort extends HasSystemBus {
  private val params = p(RecBus)
  private val device = new SimpleBus("recovery", Nil)
  val recovery_axi4 = AXI4SlaveNode(Seq(AXI4SlavePortParameters(
    slaves = Seq(AXI4SlaveParameters(
      address       = List(AddressSet(params.base, params.size-1)),
      resources     = device.ranges,
      executable    = params.executable,
...
/** Common io name and methods for propagating or tying off the port bundle */
trait HasMasterAXI4RecoveryPortBundle {
  implicit val p: Parameters
  val recovery_axi4: HeterogeneousBag[AXI4Bundle]
}
/** Actually generates the corresponding IO in the concrete Module */
trait HasMasterAXI4RecoveryPortModuleImp extends LazyModuleImp with HasMasterAXI4RecoveryPortBundle {
  val outer: HasMasterAXI4RecoveryPort
  val recovery_axi4 = IO(HeterogeneousBag.fromNode(outer.recovery_axi4.in))
  (recovery_axi4 zip outer.recovery_axi4.in) foreach { case (i, (o, _)) => i <> o }
}
