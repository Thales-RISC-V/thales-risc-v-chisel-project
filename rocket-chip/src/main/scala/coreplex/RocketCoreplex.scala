...
trait HasRocketTilesModuleImp extends LazyModuleImp
    ...
    with HasFaultTolerantPortImp
    with HasMStatusPortImp
    with HasRegisterPortsImp
    {
  ...
  outer.rocket_tiles.map(_.module).zip(rocket_tile_inputs).foreach { case(tile, wire) =>
    ...
    tile.io.overrideTvec := wire.overrideTvec
  }

  outer.rocket_tiles.map(_.module).foreach { case(tile) =>
    mstatus := tile.io.mstatus
    regs := tile.io.regs
  }
  ...
  rocket_tile_inputs.zip(outer.hartIdList).foreach { case(wire, i) =>
    ...
    wire.overrideTvec := overrideTvec
  }
}
